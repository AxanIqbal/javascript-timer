...
class App extends React.Component {
    ...
    render() {
        return <div>
            <h1>Tomato Timer</h1>
            <Grid container>
                {this.intervalTypes.map(type => <Grid item md={6}>
                        <IntervalController key={type} type={type}
            ...
            <Timer interval={this.state.isSession ? this.intervalTypes[1] : this.intervalTypes[0]} reset={this.reset}
                   time={this.state.time} start={this.countDown} isPaused={this.state.isPaused} pause={this.pause}/>
            <Footer/>
        </div>;
    }
}
